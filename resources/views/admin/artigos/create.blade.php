...
@section('content')
<div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-900">Novo Artigo</h1>
    <p class="text-gray-600">Crie um novo artigo para o blog</p>
</div>

<form method="POST" action="{{ route('admin.artigos.store') }}" enctype="multipart/form-data" class="space-y-6">
    @csrf
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="resumo" rows="3" required
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              placeholder="Breve descrição do artigo...">{{ old('resumo') }}</textarea>
                    @error('resumo')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="conteudo" id="conteudo" rows="15" required
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                              placeholder="Conteúdo completo do artigo...">{{ old('conteudo') }}</textarea>
                    @error('conteudo')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="categoria" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                ...
                                <option value="dicas" {{ old('categoria') == 'dicas' ? 'selected' : '' }}>Dicas</option>
                                <option value="mercado" {{ old('categoria') == 'mercado' ? 'selected' : '' }}>Mercado</option>
                                <option value="financiamento" {{ old('categoria') == 'financiamento' ? 'selected' : '' }}>Financiamento</option>
                        ...
                        <div>
                            ...
                            <input type="datetime-local" name="publicado_em" value="{{ old('publicado_em') }}"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
    ...
    <div class="flex justify-end space-x-4">
        ...
        <button type="submit" 
            ...
            <i class="fas fa-save mr-2"></i> Salvar Artigo
        </button>
    </div>
</form>
